...
if (!process.env.OPENROUTER_API_KEY && !process.env.OPENAI_API_KEY) {
  throw new Error("the selected model provider key is required");
}
...
function modelRates() {
  const model = configModule.config.llmJudgeModel;
...
function estimatedCost(metrics: ReturnType<typeof openai.responseMetrics>): number {
  if (metrics.costUsd > 0) return metrics.costUsd;
...
try {
  ...
  console.log(JSON.stringify({
    ...
    model: configModule.config.llmJudgeModel,
    fastTrack: {
