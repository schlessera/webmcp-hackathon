...
Split refinement supports `SEARCH_PROVIDER=parallel|openai|tavily`. Parallel
is the default regardless of which credentials are present. The historical
`openai` value names the built-in Responses web-search path; its LLM call now
runs through OpenRouter. Tavily remains the cross-room, seven-day alternative
described above. Built-in search stores validated claims but not raw snippets.
Parallel runs on its `turbo` processor by default: it costs the same per task as
`fast` and finishes sooner at some cost in quality, freeing worker slots sooner.
`PARALLEL_SEARCH_MODE=fast` trades latency for quality; request shape and the
page-fetch fallback for verbatim spans are unchanged.
...
Parallel `turbo`, $0.007 for OpenRouter's default Exa web search and $0.008 for Tavily basic.
...
decode/vision; one call for likely-row adjudication; then one Parallel `turbo`
...
split provider; without an explicit value it stays Parallel. Parallel needs
`PARALLEL_API_KEY`, uses `turbo` by default, and stores its validated snippets
per room. Tavily needs `TAVILY_API_KEY`; the `openai` compatibility value uses
OpenRouter's built-in search tool. The former combined mode remains removed.
