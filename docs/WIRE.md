...
- **Activity:** chronological events with source shapes, outcomes, decoded
  ...
  events. Counts summarize the retained recording; the result line states how
  many pass the current filters.
- **Timing:** all displayed events share a linear scale. Overlapping bars
  show concurrent work. No log scale or guessed sequential stage placement.
- **Flow:** a sequence diagram with columns for page, HTTP, socket, tools and
  ...
  The graph shows at most 50 events; search or focus narrows the diagram.
- **Inspector:** select a row or graph node, follow a connection, or focus the
  whole connected set. Timings distinguish server time, the unaccounted
...
The browser ring retains 400 events and at most 30 keepalives. Completed
events leave before open work, and eviction is counted. Notifications are
...
Instrumented POST requests opt in with `x-wire-trace: 1`. A request-scoped
recorder follows the existing async work context, including enqueued work.
The response returns at most 16 spans in a header capped at 6,000 characters.
...
and the flow diagram is bounded. Existing drawer colours and typography
...
keyboard navigation, the bounded DOM and the mobile inspector.
