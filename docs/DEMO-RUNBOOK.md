...
| 5 | Organizer | Open the organizer URL again (a fresh tab, so the page knows what it missed) | The brief opens on **While you were away**: Sarah's need by name, and Joe's move as a `?` row with no author and no content. A consent card is already waiting, badged **only you see this** |
